...
use crate::{Video, VideoDetails, VideoInfo};
...
impl VideoDescrambler {
    ...
    pub(crate) fn descramble(mut self) -> crate::Result<Video> {
        let streaming_data = self
            ...
            .ok_or_else(|| {
                Error::Custom(
                    "VideoInfo contained no StreamingData, which is essential for downloading."
                        .into(),
                )
            })?;
    ...
    fn initialize_streams(
        ...
        for raw_format in streaming_data
            .formats
            .drain(..)
            .chain(streaming_data.adaptive_formats.drain(..))
        {
            let stream =
                Stream::from_raw_format(raw_format, client.clone(), Arc::clone(video_details));
            streams.push(stream);
        }
    }
}
...
#[inline]
fn apply_descrambler_adaptive_fmts(
    streaming_data: &mut StreamingData,
    adaptive_fmts_raw: &str,
) -> crate::Result<()> {
...
fn apply_signature(streaming_data: &mut StreamingData, js: &str) -> crate::Result<()> {
        ...
        .chain(streaming_data.adaptive_formats.iter_mut())
    {
        ...
        url.query_pairs_mut().append_pair("sig", s);
    }
